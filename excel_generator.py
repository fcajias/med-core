...
def generar_excel_en_memoria(conn):
    ...
    pd_headers = [
        "ID ATENCIÓN", "FECHA", "CÉDULA", "PACIENTE", "EDAD", "CELULAR", 
        "PISO / ÁREA", "DIAGNÓSTICO", "MEDICAMENTOS ENTREGADOS", "TOTAL UNIDADES"
    ]
    ...
    cur.execute("""
        SELECT a.id, a.fecha, COALESCE(p.cedula, 'S/C'), p.nombres || ' ' || p.apellidos, 
               COALESCE(p.edad, ''), COALESCE(p.celular, ''), COALESCE(p.piso_area, ''),
               a.diagnostico
        FROM atenciones a
    ...
    for row_idx, at in enumerate(atenciones, start=2):
        aid, f, ced, pac, edad, cel, piso, diag = at
        cur.execute("""
        ...
        ws_pd.append([aid, f, ced, pac, edad, cel, piso, diag, med_text, tot_cant])
        for c_idx in range(1, 11):
            cell = ws_pd.cell(row=row_idx, column=c_idx)
            cell.font = cell_font
            cell.border = thin_border
            if c_idx in (1, 2, 3, 5, 6, 7, 10):
                cell.alignment = Alignment(horizontal="center", vertical="center")
    ...
    kar_headers = [
        ...
        "DETALLE / CONCEPTO", "CANTIDAD", "STOCK ANTERIOR", "STOCK NUEVO"
    ]
    ...
    ws_st.append(["TOP MEDICAMENTOS MÁS CONSUMIDOS (ACTUALIZADO EN VIVO)"])
    ...
    ws_st.append(["CÓDIGO", "MEDICAMENTO", "PRESENTACIÓN", "TOTAL UNIDADES DESPACHADAS"])
